...
def render_upload_panel():
    ...
    st.caption("CSV/XLSX 회원 파일 업로드 → 스냅샷 저장/비교 → 세그먼트·실행 전략까지 한 화면에서 관리합니다.")
    up1, up2 = st.columns([1.45, 1.0], gap="large")
    with up1:
        uploaded = st.file_uploader(
            "회원 CSV / XLSX 업로드",
            type=["csv", "xlsx", "xls"],
            help="CRM 파일을 업로드하면 아래 분석 탭이 활성화됩니다.",
            key="crm_upload_file",
        )
        snapshot_name = st.text_input(
            "이번 업로드 이름",
            value=datetime.now().strftime("%Y-%m-%d CRM"),
            key="crm_snapshot_name",
        )
        save_now = st.button("스냅샷 저장", key="crm_snapshot_save", use_container_width=True)
    with up2:
        st.markdown("**저장된 스냅샷**")
        ...
        prev_choice = st.selectbox(
            "비교할 이전 스냅샷",
            options=["선택 안 함"] + snapshot_labels,
            key="crm_prev_snapshot",
        )
        st.caption("카페24 실시간 연동이 없어도 주기적으로 업로드하면 추세를 이어갈 수 있습니다.")
    return uploaded, snapshot_name, save_now, prev_choice

# ---------- UI ----------

uploaded, snapshot_name, save_now, prev_choice = render_upload_panel()
